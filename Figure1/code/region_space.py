...
def _shape_space():
    ...
    # svd_solver="full", not the default. At this shape (1000 x 120) sklearn's
    # "auto" picks the RANDOMIZED solver, whose random_state defaults to None --
    # so the two shape modes came out with a sign flip depending on the global
    # numpy RNG state, and every distance downstream moved with them
    # (d[0,1] = 10.1432 or 10.1446 depending on the run). Full SVD is exact,
    # deterministic, and on a matrix this size costs nothing.
    p = PCA(2, svd_solver="full").fit(bz)
