...
# Everything this notebook reads lives under Figure2/dataset/, named by the
# paper it comes from: `rrr_selective.npz` is the 4,617 neurons that pass the
# selectivity criterion below, which is every neuron this analysis ever reads
# (it is a 14 MB slice of the 178 MB brain-wide map used elsewhere in the
# paper -- see the main repo's Data/README.md for that provenance; here it is
# just a file on disk). The two CSVs are the cortical hierarchy order and the
...
class Dataset:
    ...
    def __init__(self, npz=None, min_neurons=MIN_NEURONS):
        if npz is None:
            npz = DATASET / "posani2026" / "rrr_selective.npz"
        ...
        self.area_list = pd.read_csv(
            DATASET / "posani2026" / "area_list.csv", header=None).values[:, 0]
        self._conn = np.log(pd.read_csv(
            DATASET / "harris2019" / "ctx2ctx_conn.csv", header=None).values + 1e-8)
