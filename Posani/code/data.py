...
REPO = ROOT / "brainwide-RRR-encoding-model"
...
class Dataset:
    ...
    def __init__(self, npz=ROOT / "rrr_neurons.npz", min_neurons=MIN_NEURONS):
        ...
        self.area_list = pd.read_csv(REPO / "data/area_list.csv", header=None).values[:, 0]
        self._conn = np.log(pd.read_csv(
            REPO / "data/ctx2ctx_conn.csv", header=None).values + 1e-8)
